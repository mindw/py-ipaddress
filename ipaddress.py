...
class _IPAddressBase(_TotalOrderingMixin):
    ...
    def _prefix_from_ip_int(self, ip_int):
        ...
        if leading_ones != all_ones:
            ...
            details = int_to_packed(ip_int, byteslen, 'big')
...
class _BaseAddress(_IPAddressBase):
    ...
    def __init__(self, address):
        if (not (isinstance(address, six.binary_type) and len(address) in [4,16])
            and '/' in str(address)):
...
class IPv4Address(_BaseV4, _BaseAddress):
    ...
    def __init__(self, address):
        ...
        if isinstance(address, six.binary_type) and len(address) == 4:
...
class IPv4Interface(IPv4Address):
    ...
    def __init__(self, address):
        if isinstance(address, six.integer_types)\
            or (isinstance(address, six.binary_type) and len(address) == 4):
...
class IPv6Address(_BaseV6, _BaseAddress):
    ...
    def __init__(self, address):
        ...
        # Constructing from a packed address
        if isinstance(address, six.binary_type)\
        and (len(address) == 16)\
        and (address.find("::") == -1):
...
class IPv6Interface(IPv6Address):
    ...
    def __init__(self, address):
        if (isinstance(address, six.binary_type)\
            and (len(address) == 16)\
            and (address.find("::") == -1))\
        or isinstance(address, six.integer_types):
